...
lengthSegment2 s = sqrt . squaredNorm $ p .-. q
    where p = src s
          q = dst s

-- | Predicate that determines if a point is inside a segment.
insideSegment2 :: (Ord a, Num a) => Point2 a -> Segment2 a -> Bool
...
lineToSegment2 l@(o, _) = Segment2 p q
    where bottom = intersectLines2 l $ xaxis $ Point2 (-1, 0)
          up     = intersectLines2 l $ xaxis $ Point2 (1, 0)
